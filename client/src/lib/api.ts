...
export const createPageMutationFn = async ({
  workspaceId,
  data,
}: CreatePagePayloadType): Promise<PageResponseType> => {
  const response = await API.post(`/page/workspace/${workspaceId}/create`, data);
  return response.data;
};

export const editPageMutationFn = async ({
  pageId,
  workspaceId,
  data,
}: EditPagePayloadType): Promise<PageResponseType> => {
  const response = await API.put(
    `/page/${pageId}/workspace/${workspaceId}/update`,
    data
  );
  return response.data;
};

export const getPagesInWorkspaceQueryFn = async ({
  workspaceId,
  pageSize = 10,
  pageNumber = 1,
}: AllPagePayloadType): Promise<AllPageResponseType> => {
  const response = await API.get(
    `/page/workspace/${workspaceId}/all?pageSize=${pageSize}&pageNumber=${pageNumber}`
  );
  return response.data;
};
...
export const getPageByIdQueryFn = async ({
  workspaceId,
  pageId,
}: PageByIdPayloadType): Promise<PageResponseType> => {
  const response = await API.get(`/page/${pageId}/workspace/${workspaceId}`);
  return response.data;
};
...
export const deletePageMutationFn = async ({
  workspaceId,
  pageId,
}: PageByIdPayloadType): Promise<{
  message: string;
}> => {
  const response = await API.delete(
    `/page/${pageId}/workspace/${workspaceId}/delete`
  );
  return response.data;
};
